...
var TextArea = /** @class */ (function (_super) {
    ...
    function TextArea(props) {
            ...
            switch (mention.status) {
                ...
                case "active":
                    if (key === "Escape" ||
                        ...
                        var searchText = _this.props.value.substr(mention.startPosition + 1);
        ...
        _this.handleKeyPress = function (event) {
            var suggestionTriggerCharacters = _this.props.suggestionTriggerCharacters;
            var mention = _this.state.mention;
            var key = event.key;
            switch (mention.status) {
                case "loading":
                case "active":
                    ...
                    var searchText = _this.props.value.substr(mention.startPosition) + key;
                    _this.startLoadingSuggestions(searchText);
                    if (mention.status !== "loading") {
                        _this.setState({
                            mention: __assign(__assign({}, _this.state.mention), { status: "loading" })
                ...
                case "inactive":
                    ...
                    var caret = TextAreaCaretPosition_1.getCaretCoordinates(event.currentTarget, "@");
    ...
    TextArea.prototype.render = function () {
        ...
        return (React.createElement("div", { className: "mde-textarea-wrapper" },
            React.createElement("textarea", __assign({ className: ClassNames_1.classNames("mde-text", classes), style: { height: height }, ref: this.handleTextAreaRef, onChange: this.handleOnChange, readOnly: readOnly, value: value, "data-testid": "text-area", onBlur: suggestionsEnabled ? this.handleBlur : undefined, onKeyDown: suggestionsEnabled ? this.handleKeyDown : undefined, onKeyPress: suggestionsEnabled ? this.handleKeyPress : undefined }, textAreaProps)),
